...
function[Grid,Layer,Q] = WAKZK_Gaussian()
...
Tx.P = 100;%200;		% total acoustic power (W)
...
if Tx.d==Inf
  ...
  z_start = 0;%Tx.d-sqrt(Tx.d^2-Tx.a2^2);	% axial location of equivalent source (cm)
end
Grid.Z = 7;		% max axial distance (cm)
Grid.KK = 64;%128;		% max number of harmonics in simulation (use power of 2)
...
%% Graphical output %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
z_output = [0 5];%[4.65 4.95 5.25];	% locations on z-axis where plots are produced
LL = length(z_output);	% code determines number of plot locations
ll = 1;			% initialize index


%% Layered media %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
II = 2;%3;			% number of layers
...
Layer(2).z = 3;		% material transition distance (cm)	
Layer(2).c = 1629;
Layer(2).rho = 1000;
Layer(2).alpha = 58;
...
w = 3*Tx.a2;%1.05*a2p;		% width (radius) of physical domain (Tx radius + 5% pad)
...
A = exp(-(1/Tx.a2^2 + i*Layer(1).k/2/Tx.d)*Grid.r.^2);
...
%% Spatial averaging %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
hr = 0;%0.1*hd/2;		% convert to radius in cm 
